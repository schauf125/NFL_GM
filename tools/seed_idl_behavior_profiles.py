#!/usr/bin/env python3
"""Seed stored IDL behavior profiles for current players and draft prospects."""
...
from engine.idl_behavior import (  # noqa: E402
    IDL_STYLE_OVERRIDES,
    ensure_player_idl_behavior_schema,
    generated_idl_behavior_profile,
    normalize_name,
    profile_to_db_tuple,
...
def load_interiors(con: sqlite3.Connection, season: int) -> list[sqlite3.Row]:
    return list(
        con.execute(
            """
            SELECT p.player_id,
...
def seed_player_profiles(con: sqlite3.Connection, *, season: int, apply: bool) -> Counter[str]:
    ...
    for row in load_interiors(con, season):
        interior = player_snapshot(con, row, season)
        profile = idl_behavior_profile(interior)
        source_kind = "named" if normalize_name(interior.name) in IDL_STYLE_OVERRIDES else "inferred"
        source = f"idl_behavior_{source_kind}_seed"
        ...
        if apply:
            con.execute(
                ...
                """,
                profile_to_db_tuple(interior.player_id, season, profile, source),
            )
    return counts
...
def latest_draft_year(con: sqlite3.Connection) -> int | None:
    row = con.execute(
        ...
        WHERE dp.position IN ('IDL', 'DT', 'NT')
        """
    ).fetchone()
...
def seed_draft_profiles(
    ...
    *,
    ...
    for row in rows:
        profile = generated_idl_behavior_profile(
            str(row["archetype"] or ""),
            draft_ratings(con, int(row["prospect_id"])),
            position=str(row["position"] or "IDL"),
        )
        ...
        if apply:
            replace_prospect_idl_behavior_profile(
                ...
                DraftProspectIDLBehaviorProfile(
                    ...
                    penetration_burst=int(round(profile.penetration_burst)),
                    power_collapse=int(round(profile.power_collapse)),
                    double_team_anchor=int(round(profile.double_team_anchor)),
                    gap_control=int(round(profile.gap_control)),
                    block_shed_timing=int(round(profile.block_shed_timing)),
                    stunt_timing=int(round(profile.stunt_timing)),
                    rush_counter_plan=int(round(profile.rush_counter_plan)),
...
def main() -> int:
    ...
    for source, count in sorted(draft_counts.items()):
        print(f"{source}: {count}")
    if not args.apply:
        print("Dry run only. Add --apply to write IDL behavior profiles.")
    return 0
