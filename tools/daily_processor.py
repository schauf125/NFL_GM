...
AI_GM_EVENT_PHASES = {
    ...
    "FREE_AGENCY_NEGOTIATION_WINDOW": "free_agency",
    "NEXT_NFL_LEAGUE_YEAR_START": "free_agency",
    ...
    "REGULAR_SEASON_KICKOFF": "weekly_roster",
...
def run_ai_gm_event_reviews(con: sqlite3.Connection, *, game_id: str, event: sqlite3.Row) -> str | None:
    ...
    try:
        ...
        return f"AI GM review hook skipped: {exc}"
    review_items = len(result.get("review_items") or [])
    ...
    *,
    ...
    cur = con.execute(
        ...
        INSERT INTO game_daily_processing_runs (
            ...
            roster_error_count, roster_warning_count, injuries_resolved,
            ai_gm_hook_status, injury_hook_status
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
...
def print_range_result(result: RangeResult) -> None:
    ...
    print("  Hooks: AI GM calendar review generation enabled, development not implemented, injuries enabled")
