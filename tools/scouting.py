...
def cpu_visible_scouting_candidates(
    ...
    *,
    ...
    return con.execute(
        ...
        LIMIT ?
        """,
        (game_id, draft_year, team_id, draft_class_id, limit),
    ).fetchall()
    ...
    *,
    ...
    for team in teams:
        ...
        candidates = cpu_visible_scouting_candidates(
            ...
            team_id=team_id,
            limit=max(count_per_team * 3, count_per_team),
        )
        candidates = sorted(
            candidates,
            key=lambda row: (
                -cpu_position_need_bonus(con, team_id, str(row["position"])),
                int(row["public_board_rank"] or row["scouting_rank"] or 9999),
                -scouting_perception.perceived_grade(
                    row,
                    game_id=target_game_id,
                    draft_year=target_year,
                    team_id=team_id,
                ),
                -scouting_perception.perceived_ceiling(
                    row,
                    game_id=target_game_id,
                    draft_year=target_year,
                    team_id=team_id,
        ...
        selected = candidates[: max(1, count_per_team)]
        for prospect in selected:
            advance_cpu_prospect_one_tier(
                ...
                period=period,
                prospect=prospect,
                reason=f"{team['abbreviation']} scouts sharpened",
            )
...
def cpu_top30_weight(
    ...
    *,
    ...
    return max(0.1, confidence_bonus + rank_bonus + need_bonus + hidden_bonus + (grade - 55.0) / 8.0 + upside_bonus + variance / 10.0)
