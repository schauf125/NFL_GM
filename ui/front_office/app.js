(function () {
  ...
  function playerProfileLink(player, className) {
    const link = node("a", className || "player-link", player.name);
    link.href = `../player_profile/index.html?player=${player.id}`;
  ...
  function renderOverview(team) {
    ...
    topPlayers(team, 6).forEach((player) => {
      ...
      const left = node("div");
      append(left, [playerProfileLink(player, "player-link strong-link"), node("div", "subtle", `${player.position} | ${player.role.label}`)]);
  ...
  function renderRosterRows(team, tbody) {
    ...
    (team.roster || []).filter(rosterMatches).forEach((player) => {
      ...
      const info = node("div");
      append(info, [playerProfileLink(player, "player-link strong-link"), node("div", "subtle", `${player.jersey} | ${player.college}`)]);
    ...
    depthPositions(team).forEach((position) => {
      ...
      (team.depth[position] || []).slice(0, 5).forEach((playerId, index) => {
        ...
        append(row, [
          ...
          append(node("div"), [playerProfileLink(player, "player-link strong-link"), node("div", "subtle", player.role.label)]),
  ...
  function renderFreeAgents(team) {
    ...
    function renderRows() {
      ...
      freeAgents.filter(freeAgentMatches).slice(0, 140).forEach((player) => {
        ...
        append(row, [
          append(node("td"), [playerProfileLink(player, "player-link strong-link"), node("div", "subtle", player.previousTeam ? `Prev ${player.previousTeam}` : "Open market")]),
          node("td", null, player.position),
          node("td", null, String(player.age || "-")),
  ...
  function renderPlayerPanel(team) {
    ...
    profileLink.href = `../player_profile/index.html?player=${player.id}`;
