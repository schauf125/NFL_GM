#!/usr/bin/env python3
"""Seed stored edge behavior profiles for current players and draft prospects."""
...
from engine.edge_behavior import (  # noqa: E402
    ...
    ensure_player_edge_behavior_schema,
    generated_edge_behavior_profile,
    normalize_name,
    profile_to_db_tuple,
...
def load_edges(con: sqlite3.Connection, season: int) -> list[sqlite3.Row]:
    return list(
        con.execute(
            """
            SELECT p.player_id,
...
def seed_player_profiles(con: sqlite3.Connection, *, season: int, apply: bool) -> Counter[str]:
    ...
    for row in load_edges(con, season):
        edge = player_snapshot(con, row, season)
        profile = edge_behavior_profile(edge)
        source_kind = "named" if normalize_name(edge.name) in EDGE_STYLE_OVERRIDES else "inferred"
        source = f"edge_behavior_{source_kind}_seed"
        counts[source_kind] += 1
        if apply:
            con.execute(
                """
                INSERT INTO player_edge_behavior_profiles (
                    player_id, season, label, getoff_timing, speed_arc,
                    power_collapse, counter_plan, stunt_timing,
                    contain_discipline, run_squeeze, backside_pursuit,
                ...
                """,
                profile_to_db_tuple(edge.player_id, season, profile, source),
            )
    return counts
...
def seed_draft_profiles(
    ...
    *,
    ...
    for row in rows:
        profile = generated_edge_behavior_profile(
            str(row["archetype"] or ""),
            draft_ratings(con, int(row["prospect_id"])),
            position=str(row["position"] or "EDGE"),
        )
        ...
        if apply:
            replace_prospect_edge_behavior_profile(
                ...
                DraftProspectEdgeBehaviorProfile(
                    label=profile.label,
                    getoff_timing=int(round(profile.getoff_timing)),
                    speed_arc=int(round(profile.speed_arc)),
                    power_collapse=int(round(profile.power_collapse)),
                    counter_plan=int(round(profile.counter_plan)),
                    stunt_timing=int(round(profile.stunt_timing)),
                    contain_discipline=int(round(profile.contain_discipline)),
                    run_squeeze=int(round(profile.run_squeeze)),
                    backside_pursuit=int(round(profile.backside_pursuit)),
                    finish_skill=int(round(profile.finish_skill)),
                    rush_discipline=int(round(profile.rush_discipline)),
                    notes=profile.notes,
                ),
                ensure=False,
            )
...
def main() -> int:
    ...
    for source, count in sorted(draft_counts.items()):
        print(f"{source}: {count}")
    if not args.apply:
        print("Dry run only. Add --apply to write Edge behavior profiles.")
    return 0
