#!/usr/bin/env python3
"""Print offensive line behavior profiles used by the sim engine."""
...
def player_snapshot(con: sqlite3.Connection, row: sqlite3.Row, season: int) -> PlayerSnapshot:
    ...
    if player_ol_behavior_table_exists(con):
        ...
        if profile_row:
            metadata["ol_behavior_profile"] = profile_from_mapping(dict(profile_row))
            metadata["ol_behavior_source"] = str(profile_row["source"])
...
def load_linemen(con: sqlite3.Connection, *, season: int, team: str | None, starters_only: bool) -> list[sqlite3.Row]:
    ...
    if starters_only:
        ...
        AND EXISTS (
            ...
            WHERE dc.player_id = p.player_id
              AND dc.position IN ('LT', 'LG', 'C', 'RG', 'RT')
              AND dc.depth_rank = 1
        )
        """
    return list(
...
def row_payload(con: sqlite3.Connection, row: sqlite3.Row, season: int) -> dict[str, Any]:
    ...
    payload = {
        ...
        "overall": row["overall"],
        "style": profile.label,
        "source": ol_behavior_source(lineman),
    }
...
def draft_rows(con: sqlite3.Connection, draft_year: int) -> list[dict[str, Any]]:
    return [
        ...
        for row in con.execute(
            ...
            JOIN draft_prospect_ol_behavior_profiles olp ON olp.prospect_id = dp.prospect_id
            WHERE dc.draft_year = ?
...
def print_rows(rows: list[dict[str, Any]]) -> None:
    print(f"{'Tm':<4} {'Pos':<3} {'Lineman':<24} {'OVR':>3} {'Src':<8} {'Style':<28} {'Set':>3} {'Mir':>3} {'Anc':>3} {'Hnd':>3} {'Stn':>3} {'Drv':>3} {'Rch':>3} {'Cmb':>3} {'Pen':>3}")
    for row in rows:
        print(
            f"{row['team']:<4} {row['position']:<3} {row['name']:<24} {int(row['overall']):>3} {row['source']:<8} "
            f"{row['style']:<28} {row['pass_set_patience']:>3.0f} {row['mirror_vs_speed']:>3.0f} "
            f"{row['anchor_vs_power']:>3.0f} {row['hand_timing']:>3.0f} {row['stunt_awareness']:>3.0f} "
            f"{row['drive_finish']:>3.0f} {row['reach_range']:>3.0f} {row['combo_timing']:>3.0f} "
            f"{row['penalty_control']:>3.0f}"
        )


def print_draft_rows(rows: list[dict[str, Any]]) -> None:
    print(f"{'Pos':<3} {'Prospect':<24} {'OVR':>3} {'Archetype':<16} {'Style':<28} {'Set':>3} {'Mir':>3} {'Anc':>3} {'Hnd':>3} {'Stn':>3} {'Drv':>3} {'Rch':>3} {'Cmb':>3} {'Pen':>3}")
    for row in rows:
        print(
            f"{row['position']:<3} {row['name']:<24} {int(row['overall']):>3} {str(row['archetype'] or ''):<16} "
            f"{row['label']:<28} {int(row['pass_set_patience']):>3} {int(row['mirror_vs_speed']):>3} "
            f"{int(row['anchor_vs_power']):>3} {int(row['hand_timing']):>3} {int(row['stunt_awareness']):>3} "
            f"{int(row['drive_finish']):>3} {int(row['reach_range']):>3} {int(row['combo_timing']):>3} "
            f"{int(row['penalty_control']):>3}"
        )


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Show OL behavior profiles used by the sim engine.")
    ...
    parser.add_argument("--all", action="store_true", help="Show all linemen. Default is listed starters only.")
    parser.add_argument("--draft-year", type=int, help="Show generated draft OL profiles for this class.")
    ...
    with connect(args.db) as con:
        if args.draft_year:
            ...
            rows = [
                ...
                for row in load_linemen(con, season=args.season, team=args.team, starters_only=not args.all)
            ]
            print_rows(rows)
    if args.json:
