#!/usr/bin/env python3
"""Seed stored RB/FB behavior profiles for current players and draft prospects."""
...
from engine.rb_behavior import (  # noqa: E402
    RB_STYLE_OVERRIDES,
    ensure_player_rb_behavior_schema,
    generated_rb_behavior_profile,
    normalize_name,
    profile_to_db_tuple,
    rb_behavior_profile,
)
...
def load_backs(con: sqlite3.Connection, season: int) -> list[sqlite3.Row]:
    return list(
        con.execute(
            """
            SELECT p.player_id,
...
def seed_player_profiles(con: sqlite3.Connection, *, season: int, apply: bool) -> Counter[str]:
    ...
    for row in load_backs(con, season):
        rb = player_snapshot(con, row, season)
        profile = rb_behavior_profile(rb)
        source_kind = "named" if normalize_name(rb.name) in RB_STYLE_OVERRIDES else "inferred"
        source = f"rb_behavior_{source_kind}_seed"
        ...
        if apply:
            con.execute(
                ...
                """,
                profile_to_db_tuple(rb.player_id, season, profile, source),
            )
    return counts
...
def seed_draft_profiles(
    ...
    *,
    ...
    for row in rows:
        profile = generated_rb_behavior_profile(
            str(row["archetype"] or ""),
            draft_ratings(con, int(row["prospect_id"])),
            position=str(row["position"] or "RB"),
        )
        ...
        if apply:
            replace_prospect_rb_behavior_profile(
                ...
                DraftProspectRBBehaviorProfile(
                    label=profile.label,
                    early_down_gravity=int(round(profile.early_down_gravity)),
                    patience=int(round(profile.patience)),
                    one_cut_decisiveness=int(round(profile.one_cut_decisiveness)),
                    bounce_tendency=int(round(profile.bounce_tendency)),
                    home_run_hunting=int(round(profile.home_run_hunting)),
                    contact_appetite=int(round(profile.contact_appetite)),
                    space_creation=int(round(profile.space_creation)),
                    pass_game_usage=int(round(profile.pass_game_usage)),
                    short_yardage_trust=int(round(profile.short_yardage_trust)),
                    ball_security_mindset=int(round(profile.ball_security_mindset)),
                    notes=profile.notes,
                ),
                ensure=False,
            )
...
def main() -> int:
    ...
    for source, count in sorted(draft_counts.items()):
        print(f"{source}: {count}")
    if not args.apply:
        print("Dry run only. Add --apply to write RB behavior profiles.")
    return 0
