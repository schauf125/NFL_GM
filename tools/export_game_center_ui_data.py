...
def draft_summary(conn: sqlite3.Connection, year: int, *, user_team_id: int | None = None) -> dict[str, Any]:
    ...
    if table_exists(conn, "draft_room_pick_queue_view"):
        queue = rows_as_dicts(
            conn.execute(
                """
                SELECT *
...
def stat_leaders(conn: sqlite3.Connection, season: int) -> dict[str, Any]:
    ...
    return {
        ...
        "interceptions": player_leaders(
            conn,
            season,
            stat_keys=["interceptions", "pass_deflections", "tackles"],
...
def ai_gm_summary(conn: sqlite3.Connection, team: str | None, game_id: str | None) -> dict[str, Any]:
...
def build_payload(db_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "aiGm": ai_gm_summary(conn, user_team, game_id),
