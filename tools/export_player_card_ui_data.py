...
def fetch_rows(conn: sqlite3.Connection, season: int, limit: int | None, include_headshots: bool) -> list[sqlite3.Row]:
    headshot_join = """
...
def export(db_path: Path, output_path: Path, season: int, limit: int | None = None) -> int:
    ...
    player_rows = fetch_rows(conn, season, limit, table_exists(conn, "player_graphics_assets"))
    player_ids = [row["player_id"] for row in player_rows]
    if not player_ids:
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.write_text("window.PLAYER_CARD_DATA = [];\n", encoding="utf-8")
        return 0
