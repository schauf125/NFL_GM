...
RULE_SETS = [
    ...
    RuleSetSeed(
        ...
        notes=(
            ...
            "limit is 16, plus one qualifying international player exemption."
        ),
    ),
]
...
def action_sign_practice_squad(con: sqlite3.Connection, args: argparse.Namespace) -> None:
    ...
    limit = int(rule_set["practice_squad_limit"]) + int(rule_set["practice_squad_international_exemption_limit"])
    current_count = practice_squad_count(con, int(team["team_id"]))
    if current_count >= limit and not args.force:
        raise ValueError(f"{team['abbreviation']} practice squad is full: {current_count}/{limit}.")
...
def action_elevate_practice_squad(con: sqlite3.Connection, args: argparse.Namespace) -> None:
    ...
    if elevation_count >= 3 and not args.force:
        raise ValueError(f"{player_name(player)} already has {elevation_count} practice squad elevations this season.")
    rule_set = practice_squad_rule_set(con, season, args.phase)
    active_count = active_roster_count(con, int(team["team_id"]))
    elevation_roster_limit = int(rule_set["active_roster_limit"]) + 2
    ...
    print(
        ...
        f"({elevation_count + 1}/3, transaction {transaction_id})."
    )
