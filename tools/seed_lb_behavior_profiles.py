#!/usr/bin/env python3
"""Seed stored LB behavior profiles for current players and draft prospects."""
...
from engine.lb_behavior import (  # noqa: E402
    LB_STYLE_OVERRIDES,
    ensure_player_lb_behavior_schema,
    generated_lb_behavior_profile,
    normalize_name,
    profile_to_db_tuple,
    lb_behavior_profile,
)
...
def load_linebackers(con: sqlite3.Connection, season: int) -> list[sqlite3.Row]:
    return list(
        con.execute(
            """
            SELECT p.player_id,
...
def seed_player_profiles(con: sqlite3.Connection, *, season: int, apply: bool) -> Counter[str]:
    ...
    for row in load_linebackers(con, season):
        linebacker = player_snapshot(con, row, season)
        profile = lb_behavior_profile(linebacker)
        source_kind = "named" if normalize_name(linebacker.name) in LB_STYLE_OVERRIDES else "inferred"
        source = f"lb_behavior_{source_kind}_seed"
        counts[source_kind] += 1
        if apply:
            con.execute(
                """
                INSERT INTO player_lb_behavior_profiles (
                    player_id, season, label, trigger_quickness,
                    gap_fit_discipline, scrape_range, traffic_navigation,
                    zone_landmark_depth, man_match_carry, blitz_timing,
                    tackle_finish, rally_support, penalty_control,
                ...
                """,
                profile_to_db_tuple(linebacker.player_id, season, profile, source),
            )
    return counts
...
def latest_draft_year(con: sqlite3.Connection) -> int | None:
    row = con.execute(
        ...
        WHERE dp.position IN ('ILB', 'LB', 'OLB')
        """
    ).fetchone()
...
def seed_draft_profiles(
    ...
    *,
    ...
    for row in rows:
        profile = generated_lb_behavior_profile(
            str(row["archetype"] or ""),
            draft_ratings(con, int(row["prospect_id"])),
            position=str(row["position"] or "ILB"),
        )
        ...
        if apply:
            replace_prospect_lb_behavior_profile(
                ...
                DraftProspectLBBehaviorProfile(
                    label=profile.label,
                    trigger_quickness=int(round(profile.trigger_quickness)),
                    gap_fit_discipline=int(round(profile.gap_fit_discipline)),
                    scrape_range=int(round(profile.scrape_range)),
                    traffic_navigation=int(round(profile.traffic_navigation)),
                    zone_landmark_depth=int(round(profile.zone_landmark_depth)),
                    man_match_carry=int(round(profile.man_match_carry)),
                    blitz_timing=int(round(profile.blitz_timing)),
                    tackle_finish=int(round(profile.tackle_finish)),
                    rally_support=int(round(profile.rally_support)),
...
def main() -> int:
    ...
    for source, count in sorted(draft_counts.items()):
        print(f"{source}: {count}")
    if not args.apply:
        print("Dry run only. Add --apply to write LB behavior profiles.")
    return 0
